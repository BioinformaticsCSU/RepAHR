...
def convert_to_fastq(input_file):
    contignames, contigs = read_fasta(input_file)
    with open(input_file, 'w') as f_w:
        for name in contignames:
            seq = contigs[name]
            quality = "".join(["I" for num in range(0, len(seq))])
            f_w.write("@" + name + "\n")
            f_w.write(seq + "\n")
            f_w.write("+\n")
            f_w.write(quality + "\n")
...
def generate_repeat_spades():
    ...
    if is_paired:
        n = 1
        input_file1 = OUTPUT_PATH + "filtered_" + THRESHOLD + "_lib" + str(n) + "_1.fastq"
        input_file2 = OUTPUT_PATH + "filtered_" + THRESHOLD + "_lib" + str(n) + "_2.fastq"
        if is_fasta(input_file1):
            convert_to_fastq(input_file1)
            is_fasta_input = True
        if is_fasta(input_file2):
            convert_to_fastq(input_file2)
            is_fasta_input = True
        while n <= len(paired_file_list):
            cmd = cmd + " --pe" + str(n) + "-1 " + input_file1 + " " \
                  + " --pe" + str(n) + "-2 " + input_file2 + " "
            n += 1
        if is_fasta_input:
            cmd = cmd + "-m " + MAX_MEMORY + " -t " + THREAD + " --cov-cutoff auto --phred-offset 33 --only-assembler -o " + OUTPUT_PATH + "spades_repeat_lib"
        else:
        ...
        n = 1
        input_file = OUTPUT_PATH + "filtered_" + THRESHOLD + "_lib" + str(n) + ".fastq"
        if is_fasta(input_file):
            convert_to_fastq(input_file)
        while n <= len(single_file_list):
            cmd = cmd + " --s" + str(n) + "-1 " + input_file + " " \
                  + input_file + " "
            n += 1
        cmd = cmd + "-m " + MAX_MEMORY + " -t " + THREAD + " --cov-cutoff auto -o " + OUTPUT_PATH + "spades_repeat_lib"
        print("Running command: \n" + cmd)
